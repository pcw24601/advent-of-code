...
for line_num, ln in enumerate(lines):
    for col_number, tile_symbol in enumerate(ln.strip()):
        tiles_dict[(line_num, col_number)] = Tile(tile_symbol, line_num, col_number)

max_line_num = line_num  # Check no blank lines at end
max_col_number = col_number

beam_list = [Beam('r', 0, 0)]
...
# Display output
# for line_num in range(max_line_num):
#     for col_num in range(max_col_number):
#         tile = tiles_dict[(line_num, col_num)]
#         if tile.energised:
#             print('X', end='')
#         else:
