    ...
    for test_row, test_col in [
        ...
        if (0 <= test_row < num_rows) and (0 <= test_col < num_cols):
            ...
            if next_height == current_height + 1:
                trail_endpoints |= find_trail(grid, num_rows, num_cols, (test_row, test_col))
    return trail_endpoints
...
def main():
    ...
    answer = sum(map(lambda pos: len(find_trail(grid, num_rows, num_cols, pos)), start_positions))
    print(f'{answer=}')
