...
class Brick:
    ...
    def __repr__(self):
        return f'Brick(x1={self.x1}, y1={self.y1}, z1={self.z1}, x2={self.x2}, y2={self.y2}, z2={self.z2}, name={self.name})'
    ...
    def delete_brick(self, grid):
        grid[np.where(grid==self.name)] = 0
...
# delete each brick, then count how many more would be deleted if it disintegrated
grid_copy = deepcopy(grid)
number_deleted = 0
for index_, brick in enumerate(bricks):
    grid = deepcopy(grid_copy)
    brick.delete_brick(grid)
    for test_brick in bricks[index_+1:]:
        if not test_brick.rests_on_bricks(grid):
            # test_brick will also disintegrate
            number_deleted += 1
            test_brick.delete_brick(grid)
