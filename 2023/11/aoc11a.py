...
# transform coordinates -- add 1 on each axis for each empty row/col before that axis
new_coords = []
for coords in original_coords:
    transformed_coords = (
        coords[0] + len(empty_rows - set(range(coords[0], max_row))),
        coords[1] + len(empty_cols - set(range(coords[1], max_col)))
    )
